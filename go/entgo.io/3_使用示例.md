...
realNameData.Amount, err = global.EntMerchant.CommercialAccount.Query().
			Where(func(*sql.Selector) {
						sql.ExprP("real_name_reviews.review_status >> 32 = ?", 3)
					}).
...
tt, err := as.EntImPool.MustRead().ImAccount.Query().Where(func(s *sql.Selector) {
		s.Select().Where(sql.ExprP("id = ?", 100000000)).ForUpdate()
	}).First(ctx)
fmt.Println(tt, err)
```